...
import { retrieveAndConfirmCheckoutSessionOrder } from "@/lib/stripe/order-confirmation";
...
export default async function CheckoutSuccessPage({
  ...
  const confirmation = sessionId
    ? await retrieveAndConfirmCheckoutSessionOrder(sessionId).catch(() => null)
    : null;
  const confirmedOrder = confirmation?.isPaid ? confirmation.order : null;

  return (
    <main className="flex min-h-screen items-center justify-center bg-stone-50 px-5 py-10 text-stone-950">
      {confirmedOrder ? <ClearCartOnMount /> : null}
      <section className="w-full max-w-xl rounded-[2rem] border border-stone-200 bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-4 leading-7 text-stone-600">
          {confirmedOrder
            ? "決済が完了しました。マイページから注文履歴を確認できます。"
            : "決済結果を確認できませんでした。注文履歴で状態を確認してください。"}
        </p>
        {confirmedOrder ? (
